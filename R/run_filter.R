...
#' Filter out runs with invalid time series (Inf, all-NA, sd==0, extreme outliers)
#'
...
#' @return data.frame with Date, Flow, and run columns (inner join)
build_comparativo <- function(sim_df, obs_df, runs, floor_month = FALSE) {
  ...
  if (floor_month) {
    flow_sim <- flow_sim %>% mutate(Date = lubridate::floor_date(Date, "month"))
    obs_df   <- obs_df   %>% mutate(Date = lubridate::floor_date(Date, "month"))
  }
